...
do
  ...
  function parseBytes(json, index)
    ...
    local hex = sub(json, start, index - 1):gsub('[ \n\r\t]+', '')
    local len = #hex / 2
    local bytes = U8Arr(len)
    for i = 0, len - 1 do
      bytes[i] = bor(
...
do
  ...
  function Tibs.encode(val)
    ...
    elseif typ == 'cdata' then
      ...
      local ptr = cast(U8Ptr, val) -- input can be any cdata, not just slice8
